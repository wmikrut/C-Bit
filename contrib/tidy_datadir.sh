...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete C-bit database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no C-bit datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old C-bit datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected C-bit 0.7 datadir."
    ;;
  3)
    echo "Detected C-bit pre-0.8 datadir."
    ;;
  4)
    echo "Detected C-bit 0.8 datadir."
    ;;
esac
